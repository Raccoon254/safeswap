...
export class EmailService {
  ...
  static async sendLoginCode(email, code, userName) {
    ...
    if (!transporter) {
      ...
      console.log(`Subject: Your SafeSwap login code: ${code}`)
    ...
    try {
      ...
      const mailOptions = {
        from: `"SafeSwap" <${process.env.SMTP_FROM || process.env.SMTP_USER}>`,
        to: email,
        subject: `Your SafeSwap login code: ${code}`,
        html
      }
  ...
  static async sendWelcomeEmail(email, userName) {
    ...
    try {
      ...
      const mailOptions = {
        from: `"SafeSwap" <${process.env.SMTP_FROM || process.env.SMTP_USER}>`,
        to: email,
        subject: 'Welcome to SafeSwap - Start Trading Securely! 🎉',
        html
      }
  ...
  static async sendEscrowReceivedEmail(recipientEmail, escrowDetails, creatorEmail) {
    ...
    try {
      ...
      const mailOptions = {
        from: `"SafeSwap" <${process.env.SMTP_FROM || process.env.SMTP_USER}>`,
        to: recipientEmail,
        subject: `🎉 You received ${escrowDetails.amount} ${escrowDetails.tokenSymbol} on SafeSwap`,
        html
      }
